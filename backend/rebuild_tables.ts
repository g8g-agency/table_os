import { rebuildTableProjection } from './src/modules/tables/projections/table-runtime.projection.ts';
...
async function run() {
  ...
  for (const table of tables) {
    ...
    try {
      ...
      console.error(`Error for ${table.table_number}:`, err instanceof Error ? err.message : String(err));
    }
  }
}
run();
