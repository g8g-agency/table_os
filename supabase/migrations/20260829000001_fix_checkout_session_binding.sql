...
BEGIN
  ...
  -- 2. Validate or Resolve active session
  IF p_table_id IS NOT NULL THEN
    IF p_session_id IS NOT NULL THEN
      ...
      END IF;
    ELSE
      -- Implicitly bind to the active session for this table (prevent POS/QR race conditions where POS didn't pass session)
  ...
  -- 6. NOW Associate Order ID inside Snapshot
  ...
  -- Return minimal response; Kitchen routing is now fully delegated to TS Service Layer.
